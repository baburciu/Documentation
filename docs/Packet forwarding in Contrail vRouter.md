...
[prod] root@i1-hyp24-001:~#
```
  ### in the Oif field of above output we have the Oif:0 interface where the packet will be sent to the other compute node
  ### we also observe the Tunnel endpoint IP addresses (for MPLSoUDP, Original packet_MPLS_UDP_IP_Underlay-Ethernet).
...
```
  ...
  ### the vif0/0 (PCI: 0000:00:00.0) is the vhost0 interface of the compute host (see MAC)
...
```
  ...
  ### the br-mgmt is the EXEC-MGMT network (cat /opt/ansible/env-extra-vars/network.yml| yq -y .networks.mgmt.zones.computes_ceph_l3) address 
  ### of a nova-compute pod placed on a EXEC zone server, added as K8s worker node and using host-networking:     
